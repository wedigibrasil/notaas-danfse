...
export abstract class BaseParser {
    ...
    public abstract parse(xml: string, ibgeLookup?: IbgeLookup): Promise<unknown>;
}

export class DanfseXmlParser extends BaseParser {
    ...
    private extractEndereco(pessoa: Record<string, unknown>): { municipioUf: string; ibgeCep: string; endereco: string } {
        ...
        if (endNac) {
            ...
            const uf = String(endNac.UF || (end as Record<string, unknown> | undefined)?.UF || '') 
    ...
    public async parse(xml: string, ibgeLookup?: IbgeLookup): Promise<DanfseData> {
        ...
        const xLocEmi = String(infNFSe.xLocEmi || (emitEnderNac as Record<string, unknown>).xLocEmi || (emitEnderNac as Record<string, unknown>).xMun || '');
        ...
        const exclRedBCNum = toNum(vDescCondIncond.vDescIncond) +
        ...
        return {
            ...
            totDedRed:    fmtMoney(toNum(vDedRed.vDR) + toNum(valoresNFSe.vCalcDR) + toNum(get(ibscbsValores as Record<string, unknown>, 'vCalcReeRepRes'))),
